...
public interface GroupMapper {
    ...
    @Insert({
        "insert into todo_group (owner, name, ",
        "information, admins)",
        "values (#{owner,jdbcType=INTEGER}, #{name,jdbcType=VARCHAR}, ",
        "#{information,jdbcType=VARCHAR}, #{admins,jdbcType=VARCHAR,typeHandler=com.aye10032.tctodolist.tctodolistserver.handler.ListToVarcharTypeHandler})"
    })
    ...
    @ConstructorArgs({
        ...
        @Arg(column="information", javaType=String.class, jdbcType=JdbcType.VARCHAR),
    ...
    @Select({
        "select",
        "id, owner, name, information, admins",
        "from todo_group",
        "where id = #{id,jdbcType=INTEGER}"
    })
    @ConstructorArgs({
